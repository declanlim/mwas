...
class TestMerge:
    ...
    def left(self):
        return DataFrame(
            {
                "key": ["a", "b", "c", "d", "e", "e", "a"],
                "local": np.random.default_rng(2).standard_normal(7),
            }
        )
    ...
    def right(self):
        return DataFrame(
            {"code": np.random.default_rng(2).standard_normal(4)},
            index=["d", "b", "c", "a"],
        )
    ...
    def test_merge_join_key_dtype_cast(self):
        ...
        df1 = DataFrame({"key": [1], "local": [10]})
        df2 = DataFrame({"key": [2], "local": [20]})
        df = merge(df1, df2, how="outer")
        assert df["key"].dtype == "int64"

        df1 = DataFrame({"key": [True], "local": [1]})
        df2 = DataFrame({"key": [False], "local": [0]})
        df = merge(df1, df2, how="outer")
    ...
    def test_overlapping_columns_error_message(self):
        df = DataFrame({"key": [1, 2, 3], "local": [4, 5, 6], "code": [7, 8, 9]})
        df2 = DataFrame({"key": [1, 2, 3], "local": [4, 5, 6], "code": [7, 8, 9]})
...
class TestMergeDtypes:
    ...
    def test_join_multi_dtypes(self, d1, d2):
        ...
        right = DataFrame({"code": np.array([5, 7], dtype=dtype2)}, index=index)
        ...
        expected["code"] = np.array(np.nan, dtype=dtype2)
        expected.loc[(expected.k1 == 2) & (expected.k2 == "bar"), "code"] = 5
        expected.loc[(expected.k1 == 1) & (expected.k2 == "foo"), "code"] = 7
    ...
    def test_merge_key_dtype_cast(self):
        # GH 17044
        df1 = DataFrame({"key": [1.0, 2.0], "local": [10, 20]}, columns=["key", "local"])
        df2 = DataFrame({"key": [2], "code": [200]}, columns=["key", "code"])
        result = df1.merge(df2, on="key", how="left")
        expected = DataFrame(
            {"key": [1.0, 2.0], "local": [10, 20], "code": [np.nan, 200.0]},
            columns=["key", "local", "code"],
        )
        tm.assert_frame_equal(result, expected)
