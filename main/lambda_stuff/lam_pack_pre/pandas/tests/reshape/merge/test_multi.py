...
class TestMergeMulti:
    ...
    def test_left_join_index_preserve_order(self):
        ...
        right = DataFrame({"code": [5, 7]}, index=index)

        result = left.join(right, on=on_cols)

        expected = left.copy()
        expected["code"] = np.nan
    ...
    def test_left_merge_na_buglet(self):
        left = DataFrame(
            {
                "id": list("abcde"),
                "local": np.random.default_rng(2).standard_normal(5),
                "code": np.random.default_rng(2).standard_normal(5),
                "dummy": list("abcde"),
                "v3": np.random.default_rng(2).standard_normal(5),
            },
            columns=["id", "local", "code", "dummy", "v3"],
        )
        right = DataFrame(
            {
