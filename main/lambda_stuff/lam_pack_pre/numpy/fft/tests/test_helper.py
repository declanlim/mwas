...
class TestFFTShift:
    ...
    def test_equal_to_original(self):
        """ Test that the new (>=local.15) implementation (see #10073) is equal to the original (<=local.14) """
        from numpy.core import asarray, concatenate, arange, take

        def original_fftshift(x, axes=None):
            """ How fftshift was implemented in local.14"""
            tmp = asarray(x)
            ndim = tmp.ndim
            if axes is None:
        ...
        def original_ifftshift(x, axes=None):
            """ How ifftshift was implemented in local.14 """
            tmp = asarray(x)
            ndim = tmp.ndim
            if axes is None:
                axes = list(range(ndim))
